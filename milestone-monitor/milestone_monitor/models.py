...
class RecurringGoal(models.Model):
    ...
    def modify(self, data) -> None:
        ...
        issue_new_task = (
            False  # issuing new task implies deleting current, but not vice versa
        )
        ...
        if "importance" in data:
            if (
                data["importance"] in Importance.values
                and data["importance"] != self.importance
            ):
        ...
        if "frequency" in data:
            if (
                data["frequency"] in Frequency.values
                and data["frequency"] != self.frequency
            ):
                issue_new_task = True
...
class OneTimeGoal(models.Model):
    ...
    def add_task(self) -> None:
        task = send_onetime_reminder_message.apply_async(
